\begin{appendix} \todo{Drawings noch korrekt einfügen, sind nur mal drinn, um verweisen zu können.}
  \section{Problem Statement}
  \label{app:problem_statement}
  % \includepdf[pages=-]{appendix/problem_statement.pdf}
  \includepdf[pages=-,nup=1x2,landscape=true]{appendix/problem_statement.pdf}
  
  \section{Drawings Cameraprotection}
  \label{app:drawings_camera_protection}
  % \includepdf[pages=-]{appendix/problem_statement.pdf}
  \includepdf[pages=-,nup=1x2,landscape=true]{appendix/drawings_camera_protection.pdf}
  
  \section{Drawings Fibox}
  \label{app:drawings_fibox_bottom}
  % \includepdf[pages=-]{appendix/problem_statement.pdf}
  \includepdf[pages=-,nup=1x2,landscape=true]{appendix/drawings_fibox_bottom.pdf}
  
  \section{Drawings Mounting Adapter}
  \label{app:drawings_mounting_adapter}
  % \includepdf[pages=-]{appendix/problem_statement.pdf}
  \includepdf[pages=-,nup=1x2,landscape=true]{appendix/drawings_mounting_adapter.pdf}
  
  \section{Drawings Rear Panel}
  \label{app:drawings_rear_panel}
  % \includepdf[pages=-]{appendix/problem_statement.pdf}
  \includepdf[pages=-,nup=1x2,landscape=true]{appendix/drawings_rear_panel.pdf}
  
  \section{Drawings Side Panel}
  \label{app:drawings_side_panel}
  % \includepdf[pages=-]{appendix/problem_statement.pdf}
  \includepdf[pages=-,nup=1x2,landscape=true]{appendix/drawings_side_panel.pdf}

\end{appendix}
